...
{
    auto expected = zeroes(outputElements);
                  ...
                  const Vector<float>& previous = {})
{
    ...
    // Poisoned rather than fresh, so an element the kernel should not touch is
    // seen to be untouched and a tile that ran past an edge shows — or holding
    // what a residual product is meant to add to.
...
// Halves as a packed loader ships them: two to a word, the low half first,
// rounded the way the hardware rounds, and read back widened so the reference
// sees exactly the numbers the kernel does.
...
// grows with the magnitude of the partial sums rather than of the answer: 2048
// terms of size 2 reach far past what an answer that cancels to 0.1 suggests.
// So a tolerance relative to the answer alone is the wrong measure once the
// inner extent is long, and this adds the accumulation's own — one float
// epsilon per term, at the spread the checks here feed.
...
// Gemma's own key shape: the eight query heads share one KV head, which is a
// batch stride of zero on the key operand and nothing else — every batch reads
// the same key rows while the queries and the scores still step head by head.
// The reference walks the same strides, so what this asserts is that a shared
// operand is a stride and not a kernel.
template <typename Program>
void checkMultiQueryScores(
    int queries, int keys, int heads, int headWidth, unsigned seed)
{
    const auto queryWidth = heads * headWidth;
    const auto outputs = heads * queries * keys;

    auto shape = TiledMatMulShape::forAttentionScores(
        queries, keys, heads, headWidth, queryWidth, 0.5f, false);

    // One KV head: the key rows are headWidth wide, and every batch reads them.
    shape.bStride = headWidth;
    shape.bBatchStride = 0;

    auto queryRows = spreadValues(queries * queryWidth, seed, 2.f);
    auto keyRows = spreadValues(keys * headWidth, seed + 1u, 2.f);
    auto bias = zeroes(keys);
...
// The two stages folded into the store: the GELU after a projection, and the
// residual sum after every o_proj and down_proj, added into what the output
// held.
template <typename Program>
...
// One pipeline, two shapes, two dispatches — the layer's projections of
// several widths through one program, and what says a shape is a uniform
// rather than something the pipeline was compiled around.
template <typename Program>
