...
// The two widths Gemma's projections have, at a fraction of a prompt's rows:
// 2048 into 2048, which is q_proj and o_proj, and 2048 into 16384, which is
// the concatenated gate-and-up product's own half.
auto tSimdTiledLinearModelWidths = test("Kernels/simdTiledLinearModelWidths") = []
{
    if (!Device::shared().isValid())
        return;

    TiledProduct::checkLinear<SimdTiledLinear>(
        64, 2048, 256, 310u, TiledProduct::dotProductTolerance(2048));

    TiledProduct::checkLinear<SimdTiledLinear>(
        16, 2048, 512, 320u, TiledProduct::dotProductTolerance(2048));
};
...
// The same, at the head width Gemma has and across tile boundaries in both
// extents.
    ...
    test("Kernels/simdTiledAttentionScoresModelHeads") = []
{
    ...
    TiledProduct::checkAttentionScores<SimdTiledLinear>(70, 80, 2, 256, 510u);
};

// Multi-query: eight query heads of 256 against the one shared KV head, which
// is nothing but a zero batch stride on the key operand.
auto tSimdTiledMultiQueryScores = test("Kernels/simdTiledMultiQueryScores") = []
{
    if (!Device::shared().isValid())
        return;

    TiledProduct::checkMultiQueryScores<SimdTiledLinear>(7, 39, 8, 5, 520u);
    TiledProduct::checkMultiQueryScores<SimdTiledLinear>(9, 70, 8, 256, 530u);
};
...
// The head width the apply runs over is exactly four column tiles at 256, and
// the keys it sums across are the ragged inner extent.
    ...
    test("Kernels/simdTiledAttentionApplyModelHeads") = []
{
    ...
    TiledProduct::checkAttentionApply<SimdTiledMatMul>(70, 45, 2, 256, 610u);
};
