...
Vector<float> runMatMul(const Vector<float>& a,
                        const Vector<float>& b,
                        const Vector<float>& bias,
                        int rowCount,
                        int innerCount,
                        int columnCount)
{
    ...
    return runOverGrid(kernel, output, columnCount, rowCount);
}
...
// landing exactly on the matrix fails here rather than on the first real
// model.
...
// projection binds, since neither backend defines what a shader reading an
// unbound buffer gets.
auto tMatMulWiderShapeWithoutBias = test("Kernels/matMulWiderShapeWithoutBias") = []
{
    ...
    auto bias = zeroes(columns);

    auto result = runMatMul(a, b, bias, rows, inner, columns);
...
auto tMatMulByIdentityIsTheInput = test("Kernels/matMulByIdentityIsTheInput") = []
{
    ...
    auto result = runMatMul(a, identity, zeroes(width), rows, width, width);

    for (auto i = 0; i < result.size(); ++i)
        check(result[i] == a[i]);
};

// One pipeline, two shapes, two dispatches — what a model with eighteen blocks
// of differing widths needs, and what a kernel with a shape compiled into it
// cannot do.
