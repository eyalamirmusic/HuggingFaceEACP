...
// be when a block contributes nothing. In place, the stream has to survive its
// own read: the element stored is the one this thread read.
auto tAddZeroLeavesTheInput = test("Kernels/addZeroLeavesTheInput") = []
{
    ...
    auto stream = storageOf(a);
    auto zeroBuffer = storageOf(zeroes(elementCount));
