...
//
// Two dispatches, because a row this wide scanned by one thread is most of a
// decode step. Gemma's vocabulary is 256,000, five times Whisper's, and the
// split is what makes that a parameter rather than a rewrite: ArgmaxPartial
// spreads a row over groupsPerRow groups — 500 of them at that width — every
// thread walking its strided share in ascending order keeping the first
// maximum it meets, the group folding its candidates and writing one
// (value, index) pair. ArgmaxFinal folds a row's pairs the same way and writes
// the index.
//
...
// what torch.argmax documents. A thread or a group that met no allowed element
// carries the sentinel index and loses to any that did; a row whose every
// token is suppressed comes out as index zero, which is stated so the value is
// documented rather than incidental.
//
// The mask is one buffer and the caller decides which one, so that a
// generation config's suppression lists stay outside the kernel.
//
// The indices come out through an integer buffer, so the step that follows can
// read them as its tokens without the host in between.
...
public:
    ...
    // here: the single row of 256,000 logits a decode step ends on, which is
    // already spread over five hundred groups and fills the machine at any
    // lane count.
