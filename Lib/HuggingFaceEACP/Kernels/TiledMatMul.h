...
// attention costs no pass of its own: the score matrix is written once and
// read once, where the normalising pass over it read it twice more and wrote
// it twice.
//
// A row of A is the softmax row: batch b, row m, is row b * rows + m, which
// for an attention's apply is head h, query i, exactly as the
// [heads, queries, keys] score buffer is indexed.
enum class AFold
{
    None,
    Softmax
...
// score is in it — which is what makes exp(masked - maximum) a zero. A part of
// a tile that lies entirely past the shape reports causalMaskScore, which is
// what an empty maximum is here for the same reason it is what a masked score
// is: below every real one, and a literal rather than a limit, so what the
// shader holds is what the host wrote.
...
// to what C already holds when residual is set — the two stages either side of
// a projection, folded into its store rather than dispatched on their own —
// and with every operand addressed through a row stride and a batch stride, so
// a head's slice of a [positions, width] activation is an operand without
...
//
// The batch stride is what expresses Gemma's multi-query attention with no
// kernel of its own: eight query heads against one shared KV head is
// bBatchStride = 0, so every batch reads the same key or value rows while the
// A and C batch strides still step head by head.
...
// That is the arithmetic intensity a thread per output cannot have.
//
...
// Gemma has no bias, so every dispatch binds zeroes. The weight is the operand
// that can be packed, for MatMul's reason too, and only in the ContiguousK
// layout a shipped weight has.
...
struct TiledMatMulProgram final : ComputeProgram
{
    ...
    // The A slab's row pitch, one over the slab width so that the eight rows a
    // group reads at once land on eight different banks.
    ...
    // one does not name it, and so nothing that dispatches one has to bind it.
