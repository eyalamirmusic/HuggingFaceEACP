...
// accuracy one.
//
// **The tanh is eacp's saturatingTanh, not its tanh.** The cube grows the
...
// single NaN in the residual stream is the whole rest of the sequence. This
// file used to clamp the argument to ±10 by hand; that clamp is now eacp's own,
// which plan.md recorded as a gap, and it answers the two tails with the
// constant rather than computing them. ±10 is inside the answered range, not
// the last computed point — float32 holds nothing between tanh(9.011) and one,
// so every argument answered that way is one whose correctly rounded tanh is
...
inline Float tanhGelu(const Float& x)
{
    ...
    return 0.5f * x * (1.f + saturatingTanh(inner));
}
