...
// eacp::GPU::Buffer counts its bytes in an int, and so does every Span handed
// out of this file, so this is the ceiling on one tensor. It is plan.md's
// second eacp gap: Gemma 2B's embedding is 1.05 GB packed and 2.10 GB widened,
// both under it, and a 7B one is not.
constexpr auto largestTensorBytes =
                       ...
                       std::uint64_t blobSize)
{
    ...
    if (tensor.byteCount > largestTensorBytes)
        throw ModelError {"tensor '" + name + "' is larger than 2 GB"};
...
std::uint64_t SafeTensors::fileByteCount() const
{
...
void SafeTensors::readHeader()
{
    ...
    const auto fileSize = fileByteCount();
    ...
    // The blob is what may be gigabytes; the JSON in front of it is 13 kB for
    // a two-shard Gemma, and a Span is how it reaches the parser.
    if (headerSize > largestTensorBytes)
        throw ModelError {"safetensors header is larger than 2 GB"};
...
TensorBuffer SafeTensors::widenedBuffer(const TensorInfo& tensor) const
{
    ...
    if (byteCount > std::numeric_limits<int>::max())
        throw ModelError {"tensor '" + tensor.name
                          + "' is too large for a GPU buffer once widened"};
